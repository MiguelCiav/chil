...
pub async fn login(client: &Client, email: &str, password: &str) -> Result<(), String> {
    ...
    let resp = client.get("https://registro.scouts.org.ve/")
        .send()
        .await
    ...
    let token = {
        ...
        document.select(&token_selector)
            .next()
    ...
    let login_resp = client.post("https://registro.scouts.org.ve/users/sign_in")
        .form(&params)
        .send()
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn get_member_status(client: &Client, cedula: &str) -> Result<MemberDetails, String> {
    ...
    let result_resp = client.get("https://registro.scouts.org.ve/members/status_member_submit")
...
mod tests {
    ...
    async fn test_get_html() {
        ...
        login(&client, &email, &password).await.expect("Login failed");
