...
import { Search, Users, GraduationCap, User, Edit2, Sparkles, ArrowLeft } from 'lucide-react';
import { Card, CardHeader, CardBody, CardFooter } from '../../../../components/Card';
import { Button } from '../../../../components/Button';
import { Modal, ModalHeader, ModalBody, ModalFooter } from '../../../../components/Modal';
import { Field } from '../../../../components/Field';
import { ScoutMember } from '../../types';
import { getMembersByBatchId, updateMember } from '../../api';

interface Step3ReviewProps {
  batchId: number;
  savedMembers: ScoutMember[];
  setSavedMembers: React.Dispatch<React.SetStateAction<ScoutMember[]>>;
  onBack: () => void;
  onFinalize: () => void;
}
...
export const Step3Review: React.FC<Step3ReviewProps> = ({
  ...
  const filteredStep3Members = (activeTab === 'valid' ? validMembers : pendingMembers).filter(member => {
    const query = searchQuery.toLowerCase();
    const name = `${member.first_name} ${member.last_name}`.toLowerCase();
    const identity = member.identity.toLowerCase();
