...
describe('certificatePdfGenerator service', () => {
  ...
  describe('generateBatchCertificatesPdf', () => {
    it('generates multi-page PDF for active members and excludes pending members', async () => {
      const members = [
        mockActiveMember,
        mockPendingMember,
        {
      ...
      // 2 active members -> 1 addPage call for the second member
    ...
    it('throws an error if no active members exist in the batch', async () => {
      ...
      ).rejects.toThrow('No hay miembros activos en este lote para generar diplomas');
    });
  });
